...
async def test_get_sessions_by_user(db_session):
    ...
    sessions = await repo.get_sessions_by_user(user_id=user_id)
...
async def test_delete_all_user_sessions(db_session):
    ...
    deleted_count = await repo.delete_all_user_sessions(user_id=user_id)
    assert deleted_count == 3
    assert await repo.get_sessions_by_user(user_id=user_id) == []
...
async def test_get_active_sessions(db_session):
    ...
    active_sessions = await repo.get_active_sessions(user_id=user_id)
...
async def test_purge_expired_sessions(db_session):
    ...
    assert purged >= 1
    remaining = await repo.get_sessions_by_user(user_id=user_id)
